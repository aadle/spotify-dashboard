...
with st.container(horizontal=True):
    checkbox = st.checkbox("Show DataFrame?", value=False)

    if checkbox:
        st.dataframe(df_listening.sort_index(ascending=False))
...
st.subheader(
    f"{df_ldy.shape[0]} tracks were played in {year}", text_alignment="center"
)
with st.container():
